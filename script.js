...
document.addEventListener('DOMContentLoaded', () => {
    const sectionsToObserve = document.querySelectorAll('.about-content, .education-content, .skills-category, .portfolio-item, .contact-item');
...
function typeWriter(element, text, speed) {
    ...
    function type() {
        if (i < text.length) {
            // Add glitch effect
            if (Math.random() > 0.9) {
                element.innerHTML += String.fromCharCode(33 + Math.floor(Math.random() * 94));
            } else {
